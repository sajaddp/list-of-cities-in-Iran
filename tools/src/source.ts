...
function mapRow(raw: Record<string, unknown>, rowNumber: number): SourceRow {
  ...
  if (!row.name || !row.provinceName) throw new Error(`Missing official name at source row ${rowNumber}`);
